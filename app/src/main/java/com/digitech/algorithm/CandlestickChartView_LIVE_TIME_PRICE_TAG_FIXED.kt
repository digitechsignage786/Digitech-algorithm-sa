...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        // Live clock attached to the current price. This is visual time only:
        // OHLC values still come exclusively from real market ticks.
        ...
        timePaint.color = Color.rgb(185, 195, 205)
        timePaint.textSize = 10f
        timePaint.textAlign = Paint.Align.RIGHT

        val timeBoxWidth = 62f
        val timeBoxHeight = 17f
        val timeBoxLeft = chartRight() - timeBoxWidth
        val timeBoxTop = (labelTop - timeBoxHeight - 2f)
    ...
    private fun drawTimeScale(
        ...
        val liveText = sdf.format(java.util.Date(liveNowMillis))
    ...
    fun setLiveTickTime(timeMillis: Long) {
        liveNowMillis = timeMillis
        invalidate()
    }

    fun setLivePrice(price: Float) {
        // The forming candle itself is updated by MainActivity from real ticks.
        // This method only refreshes the visible live clock/price frame.
        if (price.isFinite()) {
            liveNowMillis = System.currentTimeMillis()
        }
        invalidate()
