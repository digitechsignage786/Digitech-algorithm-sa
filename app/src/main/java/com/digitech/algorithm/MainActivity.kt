...
class MainActivity : AppCompatActivity() {
    ...
    // Server-time anchored clock used to keep the current candle countdown
    // moving every second between real WebSocket ticks.
    private val liveClockHandler = Handler(Looper.getMainLooper())
    @Volatile private var lastServerTickMs = 0L
    @Volatile private var lastLivePrice = Float.NaN
    @Volatile private var serverClockOffsetMs = 0L
    private var liveClockRunning = false
    ...
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            ...
            val tickTime = lastServerTickMs
            val price = lastLivePrice

            // Never advance the live candle countdown during the normal
            // Saturday/Sunday market closure for the current Forex/Gold live
            // scope. The chart itself is also tick-driven, so no new candle
            // can be created by a local clock.
            val utcDay = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
                .get(Calendar.DAY_OF_WEEK)
            val weekendClosed =
                utcDay == Calendar.SATURDAY || utcDay == Calendar.SUNDAY

            if (!weekendClosed && tickTime > 0L && price.isFinite()) {
                val interval = intervalMillis(selectedTimeframe)
                val correctedNow = System.currentTimeMillis() + serverClockOffsetMs
                val bucketStart = (tickTime / interval) * interval
                val remaining = (bucketStart + interval - correctedNow)
                    .coerceIn(0L, interval)

                chart.setLivePriceCountdown(
                    price = price.toFloat(),
                    remainingMillis = remaining
                )
            } else if (weekendClosed) {
                chart.clearLivePriceCountdown()
            }

            liveClockHandler.postDelayed(this, 250L)
        }
    }
...
super.onCreate(savedInstanceState)
    ...
    private fun buildUI() {
        ...
        for (tf in timeframes) {
            ...
            button.setOnClickListener {
                ...
                if (::chart.isInitialized) chart.setLiveTimeframe(intervalMillis(selectedTimeframe))
        ...
        addToolbarButton("↗") { chart.setDrawingTool(DrawingTool.RAY) }
        addToolbarButton("—") { chart.setDrawingTool(DrawingTool.HORIZONTAL_LINE) }
        addToolbarButton("│") { chart.setDrawingTool(DrawingTool.VERTICAL_LINE) }
        addToolbarButton("□") { chart.setDrawingTool(DrawingTool.RECTANGLE) }
        addToolbarButton("○") { chart.setDrawingTool(DrawingTool.CIRCLE) }
        addToolbarButton("➤") { chart.setDrawingTool(DrawingTool.ARROW) }
        addToolbarButton("▱") { chart.setDrawingTool(DrawingTool.PARALLEL_CHANNEL) }
        addToolbarButton("F") { chart.setDrawingTool(DrawingTool.FIBONACCI) }
        addToolbarButton("✎") { chart.setDrawingTool(DrawingTool.BRUSH) }
        addToolbarButton("T") { chart.setDrawingTool(DrawingTool.TEXT) }
        addToolbarButton("↔") { chart.setDrawingTool(DrawingTool.MEASURE) }
        addToolbarButton("×") { chart.clearDrawings() }

        // ONLY the drawing toolbar is vertically scrollable.
        // The rest of the app/layout remains unchanged.
        val toolbarScroll = android.widget.ScrollView(this)
        toolbarScroll.isVerticalScrollBarEnabled = false
        toolbarScroll.isFillViewport = true
        toolbarScroll.overScrollMode = View.OVER_SCROLL_IF_CONTENT_SCROLLS
        toolbarScroll.setBackgroundColor(Color.rgb(10, 14, 19))
        ...
        chart.setLiveTimeframe(intervalMillis(selectedTimeframe))
        chart.setLiveSymbol(selectedSymbol)
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    if (weekendClosed) {
                        ...
                        statusText.setTextColor(Color.YELLOW)
                        streamStatusText.text = "MARKET CLOSED — no new live candles"
                        streamStatusText.setTextColor(Color.YELLOW)
                    } else {
                        statusText.text = "HISTORY"
                ...
                // Start the actual tick stream only when the current
                // weekend guard allows it. Closed days must not create or
                // update live candles.
                val utcDay = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
                    .get(Calendar.DAY_OF_WEEK)
                val weekendClosed =
    ...
    private fun handleLivePrice(
        ...
        try {
            ...
            // HARD CLOSED-MARKET GATE: never let a real-time tick create or
            // update a live candle while the supported Forex/Gold market is
            // closed. The countdown gate alone is not enough because the
            // candle path is driven independently by handleLivePrice().
            if (isLiveMarketClosed(tickTime)) {
                return
            }

            // Anchor the UI countdown to Twelve Data server time so the
            // seconds do not depend on how often ticks arrive.
            lastServerTickMs = tickTime
            lastLivePrice = price.toFloat()
            serverClockOffsetMs = tickTime - System.currentTimeMillis()
            startLiveCountdownClock()
    ...
    private fun startLiveCountdownClock() {
    ...
    /**
     * Returns true when the live Forex/Gold stream must be treated as closed.
     * The check is intentionally based on UTC because the provider timestamp
     * is UTC-based and the app's local timezone must not change candle state.
     *
     * Saturday/Sunday are fully closed for the currently supported live
     * Forex/Gold instruments. During those periods no incoming tick may
     * modify or create a candle.
     */
    private fun isLiveMarketClosed(tickTimeMs: Long): Boolean {
        val calendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
        if (tickTimeMs > 0L) {
            calendar.timeInMillis = tickTimeMs
        }
    ...
    private fun updateCurrentCandle(
        ...
        // Defense-in-depth: even if this method is called from another path,
        // a closed-market tick can never mutate the candle list.
        if (isLiveMarketClosed(tickTime)) return
        ...
        runOnUiThread {
            ...
            chart.setLiveTickTime(tickTime)
