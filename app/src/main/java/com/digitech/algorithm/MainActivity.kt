...
class MainActivity : AppCompatActivity() {
    ...
    // Real-time chart clock. It advances independently of price ticks so the
    // current candle countdown and time axis continue moving every second.
    private val liveClockHandler = Handler(Looper.getMainLooper())
    private var lastLivePrice = Float.NaN
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            if (isFinishing || isDestroyed) return

            val marketClosed = isMarketClosedNow()
            chart.setMarketOpen(!marketClosed)
            val now = System.currentTimeMillis()
            val interval = intervalMillis(selectedTimeframe)
            if (!marketClosed) chart.setLiveTime(now, interval)

            if (!marketClosed && lastLivePrice.isFinite()) {
                val nextBoundary = ((now / interval) + 1L) * interval
                val remaining = (nextBoundary - now).coerceAtLeast(0L)
                chart.setLivePriceCountdown(lastLivePrice, remaining)
            }

            // 250 ms keeps the seconds countdown smooth without generating
            // any synthetic market prices.
...
super.onCreate(savedInstanceState)
    ...
    private fun buildUI() {
        ...
        for (tf in timeframes) {
            ...
            button.setOnClickListener {
                ...
                chart.setLiveTime(
                    ...
                    intervalMillis(selectedTimeframe)
                )
        ...
        chart.setLiveTime(System.currentTimeMillis(), intervalMillis(selectedTimeframe))
    ...
    private fun loadMarketData() {
        ...
        thread(name = "HistoricalLoader") {
            try {
                ...
                // Never start the live stream while the locked market-closed
                // gate is active. Historical candles remain available.
                if (!isMarketClosedNow()) {
                    connectLivePriceStream(apiKey, serial)
                } else {
                    runOnUiThread {
                        if (serial == requestSerial && !isFinishing && !isDestroyed) {
                            chart.setMarketOpen(false)
                            chart.clearLivePriceCountdown()
                            statusText.text = "MARKET CLOSED"
                            statusText.setTextColor(Color.YELLOW)
                            streamStatusText.text = "MARKET CLOSED — CANDLES FROZEN"
                            streamStatusText.setTextColor(Color.YELLOW)
                        }
                    }
                }
    ...
    private fun isMarketClosedNow(): Boolean {
        val utc = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
        val day = utc.get(Calendar.DAY_OF_WEEK)
        // Current live scope: Forex/Gold. Weekend is closed; no live candle
        // may be created or modified while the market is closed.
    ...
    private fun handleLivePrice(
        ...
        if (isMarketClosedNow()) {
            // Absolutely no OHLC mutation/new candle from any late websocket tick.
            return
        }
        try {
            val json = JSONObject(message)
