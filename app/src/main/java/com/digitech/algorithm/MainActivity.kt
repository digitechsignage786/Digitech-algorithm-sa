...
class ToolScrollView(context: Context) : android.widget.ScrollView(context) {
    ...
    private fun touchSlop(): Float {
        return android.view.ViewConfiguration.get(context).scaledTouchSlop.toFloat()
    }
}
...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        // TOP BAR — all sizes are dp, not raw pixels, so text/buttons
        // do not get vertically cropped on high-density phones.
        ...
        symbolText.setPadding(dp(8), 0, dp(6), 0)
        ...
        timeframeScroll.setBackgroundColor(Color.rgb(14, 18, 24))
        ...
        timeframeBar.setPadding(dp(3), dp(2), dp(3), dp(2))
        ...
        infoBar.setPadding(dp(8), 0, dp(8), 0)
        ...
        infoBar.addView(
            statusText,
            LinearLayout.LayoutParams(dp(70), dp(30))
        )
        ...
        val toolbarScroll = ToolScrollView(this)
        toolbarScroll.isVerticalScrollBarEnabled = true
        toolbarScroll.isScrollbarFadingEnabled = false
        toolbarScroll.overScrollMode = View.OVER_SCROLL_IF_CONTENT_SCROLLS
        toolbarScroll.isSmoothScrollingEnabled = true
        toolbarScroll.setFillViewport(false)
        toolbarScroll.setBackgroundColor(Color.rgb(13, 17, 23))
        ...
        toolbar.setPadding(0, dp(3), 0, dp(18))
        toolbar.setBackgroundColor(Color.rgb(13, 17, 23))

        // TradingView-style left drawing toolbar.
        addToolbarButton("⌖", "Crosshair") { chart.setDrawingTool(DrawingTool.CROSSHAIR) }
        addToolbarButton("╱", "Trend line") { chart.setDrawingTool(DrawingTool.TREND_LINE) }
        addToolbarButton("↗", "Ray") { chart.setDrawingTool(DrawingTool.RAY) }
        addToolbarButton("—", "Horizontal line") { chart.setDrawingTool(DrawingTool.HORIZONTAL_LINE) }
        addToolbarButton("│", "Vertical line") { chart.setDrawingTool(DrawingTool.VERTICAL_LINE) }
        addToolbarButton("□", "Rectangle") { chart.setDrawingTool(DrawingTool.RECTANGLE) }
        addToolbarButton("○", "Circle") { chart.setDrawingTool(DrawingTool.CIRCLE) }
        addToolbarButton("➤", "Arrow") { chart.setDrawingTool(DrawingTool.ARROW) }
        addToolbarButton("▱", "Parallel channel") { chart.setDrawingTool(DrawingTool.PARALLEL_CHANNEL) }
        addToolbarButton("F", "Fibonacci") { chart.setDrawingTool(DrawingTool.FIBONACCI) }
        addToolbarButton("✎", "Brush") { chart.setDrawingTool(DrawingTool.BRUSH) }
        addToolbarButton("T", "Text") { chart.setDrawingTool(DrawingTool.TEXT) }
        addToolbarButton("↔", "Measure") { chart.setDrawingTool(DrawingTool.MEASURE) }
        addToolbarButton("⌫", "Delete drawings") { chart.clearDrawings() }

        toolbarScroll.addView(
            toolbar,
            android.view.ViewGroup.LayoutParams(dp(44), android.view.ViewGroup.LayoutParams.WRAP_CONTENT)
        )
        ...
        // Keep the drawing toolbar visibly on the LEFT of the chart.
        // It must never collapse into the chart or bottom bar.
        toolbar.minimumWidth = dp(44)
        ...
        // ICT ANALYSIS AREA — enough height for all status lines.
        val signalBox = LinearLayout(this)
        signalBox.orientation = LinearLayout.VERTICAL
        signalBox.gravity = Gravity.CENTER_VERTICAL
        // Wrap to the exact height of its three status rows.
        // This prevents the third line from being clipped on portrait phones.
        signalBox.setPadding(dp(8), dp(1), dp(8), dp(1))
        ...
        // Do not use a smaller fixed height here: 3 x 15dp rows plus
        // padding need about 47dp. WRAP_CONTENT prevents portrait cropping
        // while the chart above still gets all remaining space via weight.
        root.addView(
            signalBox,
            LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
    ...
    private fun addToolbarButton(label: String, description: String, action: () -> Unit) {
        val button = TextView(this)
        button.text = label
        button.textSize = 17f
        button.setTextColor(Color.LTGRAY)
        ...
        button.setOnLongClickListener {
            android.widget.Toast.makeText(this, description, android.widget.Toast.LENGTH_SHORT).show()
            true
        }
                ...
                override fun onFailure(
                    ...
                    runOnUiThread {
                        if (serial == requestSerial) {
                            ...
                            statusText.setTextColor(Color.YELLOW)
                            streamStatusText.text = "WEBSOCKET: ERROR"
                            streamStatusText.setTextColor(Color.RED)
                        }
                    }
                }
