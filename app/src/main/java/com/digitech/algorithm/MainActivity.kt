    ...
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            ...
            if (tickTime > 0L && price.isFinite()) {
                ...
                runOnUiThread {
                    if (::countdownText.isInitialized && !isFinishing && !isDestroyed) {
                        ...
                        countdownText.setTextColor(Color.rgb(205, 215, 225))
                    }
                }
            } else {
                runOnUiThread {
...
super.onCreate(savedInstanceState)
    ...
    private fun buildUI() {
        ...
        for (tf in timeframes) {
            ...
            button.setOnClickListener {
                ...
                if (::countdownText.isInitialized && lastServerTickMs > 0L) {
                    countdownText.text = "--:--"
                }
        ...
        countdownText.text = "--:--"
        countdownText.textSize = 10f
        countdownText.setTextColor(Color.LTGRAY)
        countdownText.gravity = Gravity.CENTER
        countdownText.setSingleLine(true)
        countdownText.contentDescription = "Candle countdown"
        infoBar.addView(
            countdownText,
            LinearLayout.LayoutParams(dp(48), dp(22))
        )
    ...
    private fun updateCurrentCandle(
        ...
        runOnUiThread {
            ...
            chart.setLivePriceCountdown(
                price = latest.close,
                remainingMs = remainingForTick(tickTime)
            )
    ...
    private fun remainingForTick(tickTime: Long): Long {
        val interval = intervalMillis(selectedTimeframe)
        val correctedNow = System.currentTimeMillis() + serverClockOffsetMs
        val bucketStart = (tickTime / interval) * interval
        return (bucketStart + interval - correctedNow).coerceIn(0L, interval)
    }
    ...
    private fun updatePriceHeader(
        ...
        if (::countdownText.isInitialized) {
            val remaining = remainingForTick(lastServerTickMs)
            val totalSeconds = remaining / 1000L
            countdownText.text = String.format(
                Locale.US,
                "%02d:%02d",
