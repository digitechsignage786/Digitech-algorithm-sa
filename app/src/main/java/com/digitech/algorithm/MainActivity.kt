...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        fun topButton(label: String, width: Int = 34, size: Float = 13f, action: (() -> Unit)? = null) {
            val b = TextView(this)
            b.text = label
            b.textSize = size
            ...
            topBar.addView(b, LinearLayout.LayoutParams(dp(width), dp(34)))
        }

        topButton("☰", 32, 18f) { showTools() }
        topButton("DA", 38, 15f)

        symbolText = TextView(this)
        symbolText.text = selectedSymbol
        symbolText.textSize = 15f
        ...
        symbolText.setPadding(dp(4), 0, dp(4), 0)
        symbolText.setOnClickListener { showSymbolDialog() }

        val symbolParams = LinearLayout.LayoutParams(0, dp(34), 1f)
        symbolParams.weight = 1f
        symbolParams.minimumWidth = dp(72)
        topBar.addView(symbolText, symbolParams)

        topButton("+", 32, 20f) { showSymbolDialog() }
        topButton("⌕", 32, 21f) { showSymbolDialog() }
        topButton("IND", 42, 9f) { showTools() }
        topButton("↶", 32, 18f)
        topButton("↷", 32, 18f)
        topButton("⛶", 34, 17f)
        topButton("⚙", 34, 16f) { showTools() }
        ...
        for (tf in timeframes) {
            ...
            button.setTextColor(
                if (selectedTimeframe == tf) Color.rgb(35, 170, 255)
                else Color.LTGRAY
            )
        ...
        addToolbarButton("T") { chart.setDrawingTool(DrawingTool.HORIZONTAL_LINE) }
        ...
        fun chartTool(label: String, width: Int = 42, action: (() -> Unit)? = null) {
            ...
            b.setSingleLine(true)
            if (action != null) b.setOnClickListener { action() }
            chartToolRow.addView(b, LinearLayout.LayoutParams(dp(width), dp(34)))
        }
        ...
        if (resources.configuration.orientation ==
            ...
            chartToolRow.layoutParams = android.view.ViewGroup.LayoutParams(
                android.view.ViewGroup.LayoutParams.WRAP_CONTENT, dp(31)
            )
