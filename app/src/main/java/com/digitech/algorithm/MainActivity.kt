...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        topBar.addView(logo, LinearLayout.LayoutParams(dp(38), dp(38)))
        ...
        symbolText = TextView(this).apply {
            ...
            setOnClickListener { showSymbolDialog() }
        }
        topBar.addView(symbolText, LinearLayout.LayoutParams(0, dp(38), 1f))

        fun headerButton(label: String, size: Float = 18f, action: () -> Unit): TextView {
        ...
        topBar.addView(headerButton("⌕", 22f) { showSymbolDialog() }, LinearLayout.LayoutParams(dp(38), dp(38)))
        topBar.addView(headerButton("⋮", 23f) { showTools() }, LinearLayout.LayoutParams(dp(34), dp(38)))

        root.addView(topBar, LinearLayout.LayoutParams.MATCH_PARENT, dp(46))

        // Timeframes are horizontally scrollable so small screens never crop them.
        val tfScroll = android.widget.HorizontalScrollView(this).apply {
            isHorizontalScrollBarEnabled = false
            setBackgroundColor(Color.rgb(14, 18, 24))
            clipChildren = true
        }
        val timeframeBar = LinearLayout(this).apply {
        ...
        for (tf in timeframes) {
            ...
            timeframeBar.addView(button, LinearLayout.LayoutParams(dp(52), dp(34)))
        }
        tfScroll.addView(timeframeBar, android.view.ViewGroup.LayoutParams.WRAP_CONTENT, dp(38))
        root.addView(tfScroll, LinearLayout.LayoutParams.MATCH_PARENT, dp(38))

        timeframeText = TextView(this).apply { visibility = View.GONE }
        ...
        infoBar.addView(priceText, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, 1f))
        ...
        infoBar.addView(changeText, LinearLayout.LayoutParams(dp(72), LinearLayout.LayoutParams.MATCH_PARENT))
        ...
        infoBar.addView(statusText, LinearLayout.LayoutParams(dp(76), LinearLayout.LayoutParams.MATCH_PARENT))
        root.addView(infoBar, LinearLayout.LayoutParams.MATCH_PARENT, dp(30))
        ...
        mainArea.addView(toolbar, LinearLayout.LayoutParams(dp(38), LinearLayout.LayoutParams.MATCH_PARENT))

        chart = CandlestickChartView(this)
        mainArea.addView(chart, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, 1f))

        val watchScroll = android.widget.ScrollView(this).apply {
            isVerticalScrollBarEnabled = false
            setBackgroundColor(Color.rgb(13, 17, 23))
        }
        watchlist = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
        ...
        watchlist.addView(watchTitle, LinearLayout.LayoutParams(dp(78), dp(26)))
        addWatchItem("EUR/USD")
        addWatchItem("GBP/USD")
        addWatchItem("USD/JPY")
        addWatchItem("XAU/USD")
        addWatchItem("BTC/USD")
        watchScroll.addView(watchlist)
        mainArea.addView(watchScroll, LinearLayout.LayoutParams(dp(82), LinearLayout.LayoutParams.MATCH_PARENT))

        root.addView(mainArea, LinearLayout.LayoutParams.MATCH_PARENT, 0, 1f)

        val signalScroll = android.widget.HorizontalScrollView(this).apply {
            isHorizontalScrollBarEnabled = false
            setBackgroundColor(Color.rgb(17, 22, 29))
        }
        val signalBox = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(dp(10), dp(4), dp(10), dp(4))
        }
        val signalTitle = TextView(this).apply {
            text = "ICT AUTO ANALYSIS"
            textSize = 10f
            setTextColor(Color.LTGRAY)
            includeFontPadding = false
        }
        streamStatusText = TextView(this).apply {
        ...
        val signal = TextView(this).apply {
            text = "Waiting for live market data..."
            textSize = 11f
            setTextColor(Color.WHITE)
            includeFontPadding = false
            isSingleLine = true
        ...
        signalScroll.addView(signalBox)
        root.addView(signalScroll, LinearLayout.LayoutParams.MATCH_PARENT, dp(58))

        val bottomBar = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER
            setBackgroundColor(Color.rgb(18, 22, 29))
        }
        ...
        root.addView(bottomBar, LinearLayout.LayoutParams.MATCH_PARENT, dp(44))
