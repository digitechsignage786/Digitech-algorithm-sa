...
class MainActivity : AppCompatActivity() {
    ...
    // Keeps the live MM:SS clock moving even when the market sends no tick
    // for a moment. It never creates fake price data.
    private val liveClockHandler = Handler(Looper.getMainLooper())
    private var lastLivePrice = Float.NaN
    private var lastLiveTickMs = 0L
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            if (isFinishing || isDestroyed) return
            val now = System.currentTimeMillis()
            chart.setLiveTime(now, intervalMillis(selectedTimeframe))
            if (lastLivePrice.isFinite()) {
                val interval = intervalMillis(selectedTimeframe).coerceAtLeast(1L)
                val nextBoundary = ((now / interval) + 1L) * interval
                val remaining = (nextBoundary - now).coerceAtLeast(0L)
                chart.setLivePriceCountdown(lastLivePrice.toFloat(), remaining)
            }
...
super.onCreate(savedInstanceState)
    ...
    private fun handleLivePrice(
        ...
        try {
            ...
            // Show every real tick immediately in the price header and bind
            // the same real price to the live candle/countdown marker.
            lastLivePrice = price
            lastLiveTickMs = tickTime
            val intervalNow = intervalMillis(selectedTimeframe).coerceAtLeast(1L)
            val nextBoundary = ((tickTime / intervalNow) + 1L) * intervalNow
            val remainingMs = (nextBoundary - tickTime).coerceAtLeast(0L)

            runOnUiThread {
                if (serial == requestSerial && !isFinishing && !isDestroyed) {
                    priceText.text = String.format(Locale.US, "%.5f", price)
                    chart.setLiveTickTime(tickTime)
                    chart.setLiveTimeframe(intervalNow)
                    chart.setLivePriceCountdown(price.toFloat(), remainingMs)
                }
            }
