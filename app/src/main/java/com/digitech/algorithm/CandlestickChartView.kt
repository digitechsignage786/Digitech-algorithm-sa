...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live clock used by the chart time axis. It advances every second even
    // when the market sends no new tick, so the current minute/second keeps
    // moving visually like a live trading chart.
    ...
    /**
     * Draws the continuously moving current time on the right side of the
     * time axis. The candle timestamps remain historical/open times; this
     * marker represents "now" and advances once per second.
     */
    private fun drawLiveTimeMarker(canvas: Canvas) {
        if (candles.isEmpty()) return

        val last = candles.last()
        val interval = if (candles.size >= 2) {
            (candles.last().time - candles[candles.lastIndex - 1].time)
                .coerceAtLeast(60_000L)
        } else {
            5L * 60L * 1000L
        }

        val elapsed = (liveNowMillis - last.time).coerceAtLeast(0L)
        ...
        canvas.drawText(
            sdf.format(java.util.Date(liveNowMillis)),
            chartRight() - 4f,
            height - 3f,
            timePaint
        )

        // Small countdown under the current price line, so the user can see
        // exactly how much time is left in the forming candle.
        val remaining = (last.time + interval - liveNowMillis)
            .coerceAtLeast(0L)
        val totalSeconds = (remaining + 999L) / 1000L
        val mm = (totalSeconds / 60L) % 60L
        val ss = totalSeconds % 60L

        val countdownPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(130, 150, 175)
            textSize = 9f
            textAlign = Paint.Align.LEFT
        }

        canvas.drawText(
            String.format(Locale.US, "LIVE %02d:%02d", mm, ss),
            chartLeft() + 4f,
            height - 3f,
            countdownPaint
        )
