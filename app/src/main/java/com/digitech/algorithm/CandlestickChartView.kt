...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun syncDrawingsToChartTransform() {
        ...
        val sx = newStep / oldStep
        val sy = oldRange / newRange
        val chartL = chartLeft()
        val chartT = chartTop()
        val chartH = (chartBottom() - chartT).coerceAtLeast(1f)
        val yShift = (newHigh - oldHigh) * chartH / newRange

        fun tx(x: Float): Float = chartL + (x - chartL - oldOffset) * sx + offsetX
        fun ty(y: Float): Float = chartT + (y - chartT) * sy + yShift
