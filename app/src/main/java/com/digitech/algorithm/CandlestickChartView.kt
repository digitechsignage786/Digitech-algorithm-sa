...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            ...
            // Redraw frequently so the current candle/price marker reacts
            // immediately when the latest REAL tick has changed. No price is
            // generated here; only the last received market price is used.
            if (livePriceOverride.isFinite()) {
                applyLatestRealPriceToCurrentCandle(livePriceOverride)
            }
            invalidate()
    ...
    fun setLivePriceCountdown(
        ...
        if (price.isFinite()) {
            ...
            // Keep the currently forming candle synchronized with the latest
            // REAL WebSocket price even when MainActivity has not delivered a
            // new candle snapshot yet. This does not interpolate or invent a
            // price: the exact last received market tick is applied as-is.
            applyLatestRealPriceToCurrentCandle(price)
        }
        livePriceCountdownMillis = remainingMillis.coerceAtLeast(0L)
        invalidate()
    }

    private fun applyLatestRealPriceToCurrentCandle(price: Float) {
        if (!price.isFinite() || candles.isEmpty()) return
