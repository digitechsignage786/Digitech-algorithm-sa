...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private val circleRects = mutableListOf<RectData>()
    private val measurements = mutableListOf<MeasureData>()
    private val fibonacciLines = mutableListOf<MeasureData>()
    ...
    private fun drawStoredDrawings(
        ...
        for (line in rayLines) {
            ...
            val endX = if (kotlin.math.abs(dx) < 0.001f) line.x2 else if (dx > 0) width.toFloat() else 0f
            val endY = if (kotlin.math.abs(dx) < 0.001f) line.y2 else line.y1 + dy / dx * (endX - line.x1)
            canvas.drawLine(line.x1, line.y1, endX, endY, drawingPaint)
        }

        drawingPaint.color = Color.rgb(210, 150, 70)
        ...
        drawingPaint.color = Color.rgb(80, 180, 235)
        for (rect in circleRects) {
            canvas.drawOval(rect.left, rect.top, rect.right, rect.bottom, drawingPaint)
        }

        drawingPaint.color = Color.rgb(190, 150, 70)
        for (fib in fibonacciLines) {
            val y1 = fib.y1
            val y2 = fib.y2
            ...
            val high = max(y1, y2)
            for (level in levels) {
                val y = low + (high - low) * level
                canvas.drawLine(min(fib.x1, fib.x2), y, max(fib.x1, fib.x2), y, drawingPaint)
            }
        }

        drawingPaint.color = Color.rgb(200, 200, 200)
        for ((x, y) in textMarks) {
            canvas.drawText("DA", x, y, drawingPaint)
        }
    ...
    private fun drawActiveDrawing(
        ...
        when (activeTool) {
            ...
            DrawingTool.PARALLEL_CHANNEL -> {
                ...
                if (activeTool == DrawingTool.ARROW) {
                    val angle = kotlin.math.atan2(currentY - startY, currentX - startX)
                    val size = 14f
                    canvas.drawLine(currentX, currentY, currentX - size * kotlin.math.cos(angle - 0.5), currentY - size * kotlin.math.sin(angle - 0.5), drawingPaint)
                    canvas.drawLine(currentX, currentY, currentX - size * kotlin.math.cos(angle + 0.5), currentY - size * kotlin.math.sin(angle + 0.5), drawingPaint)
                }
            }
            ...
            DrawingTool.FIBONACCI -> {
                drawingPaint.color = Color.rgb(190, 150, 70)
                val low = min(startY, currentY)
                val high = max(startY, currentY)
                val levels = floatArrayOf(0f,0.236f,0.382f,0.5f,0.618f,0.786f,1f)
                for (level in levels) {
                    val y = low + (high-low)*level
                    canvas.drawLine(min(startX,currentX), y, max(startX,currentX), y, drawingPaint)
                }
            }

            DrawingTool.BRUSH -> {
                drawingPaint.color = Color.rgb(55, 180, 235)
                drawingPaint.strokeWidth = 3f
                canvas.drawLine(startX, startY, currentX, currentY, drawingPaint)
            }

            DrawingTool.TEXT -> {
                drawingPaint.color = Color.LTGRAY
                drawingPaint.textSize = 28f
                canvas.drawText("DA", currentX, currentY, drawingPaint)
            }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (
                    ...
                    when (activeTool) {
                        ...
                        DrawingTool.TREND_LINE -> {
                            trendLines.add(LineData(startX, startY, event.x, event.y))
                        }

                        DrawingTool.RAY -> {
                            rayLines.add(LineData(startX, startY, event.x, event.y))
                        }

                        DrawingTool.ARROW, DrawingTool.PARALLEL_CHANNEL -> {
                            trendLines.add(LineData(startX, startY, event.x, event.y))
                        }

                        DrawingTool.HORIZONTAL_LINE -> {
                            horizontalLines.add(event.y)
                        }

                        DrawingTool.VERTICAL_LINE -> {
                            verticalLines.add(event.x)
                        }

                        DrawingTool.RECTANGLE -> {
                            rectangles.add(RectData(min(startX,event.x), min(startY,event.y), max(startX,event.x), max(startY,event.y)))
                        }

                        DrawingTool.CIRCLE -> {
                            circleRects.add(RectData(min(startX,event.x), min(startY,event.y), max(startX,event.x), max(startY,event.y)))
                        }

                        DrawingTool.FIBONACCI -> {
                            fibonacciLines.add(MeasureData(startX, startY, event.x, event.y))
                        }
                        ...
                        DrawingTool.TEXT -> {
                            textMarks.add(Pair(event.x, event.y))
                        }

                        DrawingTool.MEASURE -> {
                            measurements.add(MeasureData(startX, startY, event.x, event.y))
                        }
