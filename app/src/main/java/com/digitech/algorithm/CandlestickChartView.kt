...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun updateLiveFollowOffset() {
        ...
        // When the market is closed, do not advance the time fraction.
        // This prevents the last candle from visually sliding/forming across
        // the closed period.
        val elapsed = if (marketOpen) {
            (liveNowMillis - lastTime).coerceAtLeast(0L)
        } else {
            0L
        }
    ...
    fun setMarketOpen(open: Boolean) {
        ...
        marketOpen = open
        if (!open) {
            // Freeze all time-driven visual movement at the last known state.
