...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Real-time clock used to animate the current candle/time marker.
    private var liveClockMillis = System.currentTimeMillis()
    private var liveIntervalMillis = 5 * 60_000L
    ...
    fun setLiveTime(
        ...
        liveIntervalMillis = intervalMillis.coerceAtLeast(1L)
        invalidate()
    }
    ...
    private fun drawLiveTimeMarker(
        canvas: Canvas
    ) {
        ...
        val elapsed = (liveClockMillis - latest.time)
            .coerceIn(0L, liveIntervalMillis)

        val progress =
            elapsed.toFloat() / liveIntervalMillis.toFloat()

        // The marker travels across the current candle's time slot
        // from its opening second to the end of the selected timeframe.
        val x =
            candleX(candles.lastIndex) +
            widthOfCandle / 2f +
            step * progress

        if (x < chartLeft() - 2f || x > chartRight() + 2f) return

        val markerPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(45, 190, 255)
            strokeWidth = 1.5f
        ...
        val timeText =
            java.text.SimpleDateFormat(
                "HH:mm:ss",
                java.util.Locale.US
            ).format(java.util.Date(liveClockMillis))
        ...
        val boxTop = height - 34f

        val timeBoxPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(25, 34, 46)
