        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                if (activeTool != DrawingTool.CROSSHAIR) {
                    ...
                    if (!crosshairVisible) {
                        // Do NOT show crosshair on ACTION_DOWN.
                        // A quick move is reserved for normal chart panning.
                        // Crosshair appears only after a short hold.
                        ...
                        val runnable = Runnable {
                            if (
                                ...
                                crosshairDragMode = CrosshairDragMode.BOTH
                                panGestureStarted = false
                                manualPanCandidate = false
                                dragging = true
                                crosshairHoldPending = false
                        ...
                        crosshairCreatedThisGesture = false
                        // Selective control: only a deliberate touch near a
                        // level line grabs that line. Touching elsewhere on the
                        // chart remains available for normal one-finger panning.
                        // Keep the hit zone compact so the two level lines do
                        // not accidentally capture ordinary chart drags.
                        val nearVertical = kotlin.math.abs(touchX - crossX) <= tolerance
                        val nearHorizontal = kotlin.math.abs(touchY - crossY) <= tolerance
                        ...
                        when {
                            ...
                            nearVertical -> {
                                // Touch the vertical line to move TIME only.
                                crosshairDragMode = CrosshairDragMode.VERTICAL
                                dragging = true
                            }
                            nearHorizontal -> {
                                // Touch the horizontal line to move PRICE only.
                                crosshairDragMode = CrosshairDragMode.HORIZONTAL
                                dragging = true
                            }
                            else -> {
                                // Crosshair is persistent, but the rest of the chart
                                // must remain freely pannable with one finger.
                                // Touching a line moves that line; touching away from
                                // both lines starts normal chart panning. A clean tap
                                // (without movement) is still handled on ACTION_UP
                                // and dismisses the crosshair.
                                crosshairDragMode = CrosshairDragMode.NONE
                                dragging = true
                                panGestureStarted = false
                                manualPanCandidate = true
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (
                    ...
                    if (kotlin.math.hypot(totalDx.toDouble(), totalDy.toDouble()) > 2.5) {
                        crosshairHoldPending = false
                        crosshairHoldRunnable?.let { removeCallbacks(it) }
                ...
                if (activeTool != DrawingTool.CROSSHAIR) {
                    ...
                    // If the crosshair is visible and the finger started away
                    // from both level lines, keep the crosshair visible while
                    // allowing the chart itself to pan freely in every direction.
                    if (
                        ...
                        when (crosshairDragMode) {
                            CrosshairDragMode.BOTH -> {
                                crossX = event.x.coerceIn(chartLeft(), chartRight())
                                crossY = event.y.coerceIn(chartTop(), chartBottom())
                            }
                            CrosshairDragMode.VERTICAL -> {
                                // Move TIME only; keep PRICE exactly where it is.
                                crossX = event.x.coerceIn(chartLeft(), chartRight())
                            }
                            CrosshairDragMode.HORIZONTAL -> {
                                // Move PRICE only; keep TIME exactly where it is.
                                crossY = event.y.coerceIn(chartTop(), chartBottom())
                            }
                            CrosshairDragMode.NONE -> Unit
                        }
                    }
                }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (activeTool == DrawingTool.CROSSHAIR && crosshairVisible) {
                    ...
                    if (!moved && !crosshairCreatedThisGesture) {
                        // Second tap removes both levels.
