...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live chart time is driven ONLY by real market ticks.
    // No local timer is allowed to advance the candle/time axis when the
    // market/data feed is closed or when no real tick has arrived.
    private var liveNowMillis: Long = 0L
    ...
    // Intentionally no synthetic live clock. Real ticks call setLiveTickTime().
    ...
    private fun isWeekendMarketClosed(): Boolean {
        val utcDay = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
            .get(Calendar.DAY_OF_WEEK)
        return utcDay == Calendar.SATURDAY || utcDay == Calendar.SUNDAY
    }
    ...
    fun setCandles(
        ...
        if (candles.isEmpty()) {
            ...
            // Before the first real tick, anchor the live clock to the last
            // real/historical candle instead of the phone clock. This prevents
            // the chart from visually progressing on weekends/holidays.
            if (liveTickTimeMillis <= 0L) {
                liveNowMillis = candles.last().time
            }

            if (followLive && !manualPanCandidate) {
                // Live-follow moves only when a real tick changes
                // liveNowMillis. There is no synthetic timer movement.
                updateLiveFollowOffset()
            }
        }
    ...
    fun setLiveTickTime(tickTimeMillis: Long) {
        if (tickTimeMillis <= 0L || isWeekendMarketClosed()) return

        // The chart clock advances only from a REAL market tick and never
        // during the closed weekend period.
        liveTickTimeMillis = tickTimeMillis
        liveNowMillis = tickTimeMillis
        invalidate()
    }
