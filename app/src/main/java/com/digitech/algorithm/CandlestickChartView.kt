...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawTimeScale(
        ...
        val sdf = java.text.SimpleDateFormat(if (interval <= 60_000L) "HH:mm:ss" else "HH:mm", java.util.Locale.US)
        ...
        // Always show the current clock at the live edge. For 1m we show
        // seconds too; for larger timeframes the minute clock matches the
        // candle grid. This is display-only and does not create fake candles.
        val now = System.currentTimeMillis()
        val nowText = sdf.format(java.util.Date(now))
        canvas.drawText(nowText, right - 8f, height - 14f, textPaint)

        // Show how far the current candle has progressed, e.g. 04:12 / 05:00.
        val currentOpen = latestTime
        val elapsed = (now - currentOpen).coerceIn(0L, interval)
        val remain = (interval - elapsed).coerceAtLeast(0L)
        val remainSec = remain / 1000L
