...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun moveToLatest() {
        ...
        val visibleWidth = max(chartRight() - chartLeft(), 1f)
        val lastCandleX = chartLeft() +
            (candles.size - 1) * candleStep() +
            candleWidth()

        offsetX = visibleWidth - (lastCandleX - chartLeft())
    }
    ...
    private fun visibleRange(): Pair<Float, Float> {
        ...
        // Determine the actual candles whose bodies/wicks are inside the
        // current viewport. Do not fall back to the entire 5000-candle
        // history merely because an edge calculation misses by a few pixels.
        for (i in candles.indices) {
            val x = candleX(i)

            if (x + widthOfCandle < left || x > right) {
                continue
            }
        ...
        // Safety fallback: use the last visible-sized block of candles,
        // rather than the entire history. This keeps the price scale useful.
        if (!found) {
            val visibleCount = max(20, ((right - left) / step).toInt() + 5)
            val from = max(0, candles.size - visibleCount)
        ...
        val rawRange = max(highest - lowest, 0.000001f)
        val padding = rawRange * 0.10f
    ...
    fun setCandles(
        ...
        // Always open a newly loaded symbol/timeframe at the live/latest side.
        moveToLatest()

        invalidate()
    }

    fun keepLatestVisible() {
        moveToLatest()
        invalidate()
    }
