...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live clock: advances every second without fabricating market prices.
    // Real price/candle changes still come only from WebSocket ticks.
    ...
    private fun drawTimeScale(
        ...
        for (i in candles.indices step every) {
            ...
            canvas.drawText(
                sdf.format(Date(candles[i].time)),
                x,
                height - 14f,
                textPaint
            )
        ...
        // TradingView-style live clock. It moves every second while the
        // chart remains open, so the user can see the market time advancing.
        // This is display-only: it never creates or modifies OHLC data.
        val clock = SimpleDateFormat("HH:mm:ss", Locale.US)
        textPaint.textAlign = Paint.Align.RIGHT
        textPaint.textSize = 10f
        textPaint.color = Color.rgb(155, 166, 180)

        val clockText = clock.format(Date(liveClockMillis))
        val clockX = chartRight() - 4f
        val clockY = height - 14f

        val bg = Paint(Paint.ANTI_ALIAS_FLAG)
        bg.color = Color.rgb(18, 25, 35)
        bg.style = Paint.Style.FILL
        val w = textPaint.measureText(clockText) + 10f
        canvas.drawRect(
            clockX - w,
            clockY - 12f,
            clockX + 2f,
        ...
        canvas.drawText(clockText, clockX - 4f, clockY, textPaint)
