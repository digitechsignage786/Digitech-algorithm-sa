...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Latest REAL market tick supplied by MainActivity. No synthetic price is generated.
    ...
    private fun drawTimeScale(
        ...
        // Live timeline: show the REMAINING TIME in the selected timeframe
        // at the live edge. For example:
        //   1m  -> 00:59 ... 00:00
        //   5m  -> 04:59 ... 00:00
        //   1H  -> 59:59 ... 00:00
        //   4H  -> 03:59:59 ... 00:00:00
        // This is driven by the real wall clock, not by synthetic prices.
        val interval = lastCandleIntervalMillis.coerceAtLeast(1L)

        // MainActivity supplies the exact remaining time for the selected
        // timeframe every 250 ms. Use that value first so the chart countdown
        // is synchronized with the live-price clock.
        val remaining = if (livePriceCountdownMillis > 0L) {
            livePriceCountdownMillis
        } else {
            val elapsedInInterval = ((liveNowMillis % interval) + interval) % interval
            val calculated = interval - elapsedInInterval
            if (calculated == interval) interval else calculated
        }.coerceAtLeast(0L)

        val totalSeconds = (remaining + 999L) / 1000L
        val liveText = if (interval >= 60L * 60L * 1000L) {
        ...
        // Prominent live countdown. It is tied to the selected timeframe:
        // 1m=00:59..00:00, 5m=04:59..00:00, 15m=14:59..00:00,
        // 30m=29:59..00:00, 1H=00:59:59..00:00:00, etc.
        val prefix = "LIVE  "
        val timeLabelWidth = if (interval >= 60L * 60L * 1000L) 108f else 82f
        val timeLabelHeight = 22f
        val timeLabelLeft = (chartRight() - timeLabelWidth).coerceAtLeast(chartLeft() + 4f)
        val timeLabelTop = chartTop() + 6f

        val timeBg = Paint(Paint.ANTI_ALIAS_FLAG)
        timeBg.color = Color.rgb(20, 28, 39)
    ...
    /**
     * Receives the real wall-clock time from MainActivity.
     * This only controls the chart clock/time-axis animation; it never creates
     * or changes OHLC values.
     */
    fun setLiveTime(
        nowMillis: Long,
    ...
    /**
     * Receives the latest REAL market price and the remaining time in the
     * current candle. The value is used for the live price line/label only.
     * No synthetic price movement is generated.
     */
