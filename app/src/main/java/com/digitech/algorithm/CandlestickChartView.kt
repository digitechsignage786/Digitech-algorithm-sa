...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private var liveClockMillis: Long = System.currentTimeMillis()
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            liveNowMillis = System.currentTimeMillis()
            liveClockMillis = liveNowMillis
            invalidate()
    ...
    private fun drawTimeScale(
        ...
        // A second-level live clock at the price edge. It is independent of
        // incoming ticks, so MM:SS keeps moving even between market ticks.
        val intervalNow = liveClockMillis.coerceAtLeast(0L)
        val remaining = ((
            ((intervalNow / liveIntervalMs) + 1L) * liveIntervalMs
        ) - intervalNow).coerceAtLeast(0L)
    ...
    /** Backward-compatible alias used by older MainActivity builds. */
    fun setLiveTime(timeMillis: Long, intervalMillis: Long) {
        liveNowMillis = timeMillis
        liveClockMillis = timeMillis
        liveIntervalMs = intervalMillis.coerceAtLeast(1L)
