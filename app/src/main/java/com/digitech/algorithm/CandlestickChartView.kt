    ...
    private val scaleDetector = ScaleGestureDetector(
        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScale(
                ...
                // Pinch zoom is deliberately centered on the chart itself.
                // This prevents the two-finger gesture from dragging the chart
                // toward an edge or creating a large blank area. The pinch only
                // changes scale; it does not change the chart's pan position.
                ...
                val anchorX = (chartLeft() + chartRight()) * 0.5f
                val anchorY = (chartTop() + chartBottom()) * 0.5f

                if (newZoom != oldZoom) {
                    val ratio = newZoom / oldZoom
                    // Keep the chart's current center fixed. Do not use the
                    // finger midpoint as an anchor because that was causing
                    // the chart to travel outward during pinch gestures.
                    offsetX = anchorX - (anchorX - offsetX) * ratio
                    scaleDrawings(ratio, 1f, anchorX, anchorY)
                    zoom = newZoom
                }

                if (newPriceZoom != oldPriceZoom) {
                    val ratio = newPriceZoom / oldPriceZoom
                    // Keep the price field centered as it zooms vertically.
                    // pricePan itself is intentionally unchanged.
                    scaleDrawings(1f, ratio, anchorX, anchorY)
                    priceZoom = newPriceZoom
                }

                // A pinch must never introduce a new pan. Clamp only the final
                // geometry as a safety net; because offsetX/pricePan stay
                // center-anchored, this cannot produce the old outward jump.
                val oldOffset = offsetX
                val oldPan = pricePan
                clampFreePan()

                val correctionDx = offsetX - oldOffset
                val correctionDy = pricePanToScreenDy(pricePan - oldPan)
