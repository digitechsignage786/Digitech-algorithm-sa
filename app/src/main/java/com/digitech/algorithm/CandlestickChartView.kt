...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live clock used for TradingView-style continuously advancing time.
    // It moves every second even between market ticks.
    private var liveTimeMillis = System.currentTimeMillis()
    ...
    private fun drawLiveClock(
        ...
        // Keep the live clock inside the current candle's time window.
        val intervalMillis = currentIntervalMillis()
        val elapsed = (liveTimeMillis - last.time)
            .coerceIn(0L, intervalMillis)

        val fraction =
            if (intervalMillis > 0L) {
                elapsed.toFloat() / intervalMillis.toFloat()
            } else {
                0f
            }

        // The marker travels from the start of the current candle toward
        // its right edge as seconds pass, without changing candle width.
        val markerX =
            candleX(lastIndex) +
            candleWidth() * fraction.coerceIn(0f, 1f) +
            candleWidth() / 2f

        if (markerX < chartLeft() || markerX > chartRight()) return

        val liveLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(95, 115, 140)
            strokeWidth = 1f
        ...
        val timeText = sdf.format(
            java.util.Date(liveTimeMillis)
        )
        ...
        val labelHeight = 19f
        val labelLeft =
            (markerX - labelWidth / 2f)
                .coerceIn(chartLeft(), chartRight() - labelWidth)

        val labelTop = chartBottom() - 21f

        val boxPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(28, 39, 54)
    ...
    private fun currentIntervalMillis(): Long {
        val stepMinutes = when {
            candles.size >= 2 -> {
                val diff = candles[candles.lastIndex].time -
                    candles[candles.lastIndex - 1].time
                (diff / 60_000L).coerceAtLeast(1L)
            }
            else -> 5L
        }

        return stepMinutes * 60_000L
    }
    ...
    fun setLiveTime(timeMillis: Long) {
        liveTimeMillis = timeMillis
        invalidate()
    }
    ...
    fun setCandles(
        ...
        if (candles.isNotEmpty()) {
            // Keep the live clock aligned with the newest real candle when
            // history is loaded, while the MainActivity advances it every
            // second afterward.
            liveTimeMillis = maxOf(
                liveTimeMillis,
                candles.last().time
            )
        }
