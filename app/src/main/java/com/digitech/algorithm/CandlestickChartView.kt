...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        // Prefer the latest REAL tick. If no tick has arrived yet, use the
        // forming candle close.
        ...
        // This Y coordinate is calculated directly from livePrice, so the
        // countdown is anchored to EXACTLY the same price level.
        val y = chartTop() +
        ...
        // The countdown is derived from the SAME live clock and selected
        // timeframe interval used by the chart. This keeps it synchronized
        // even when no new market tick arrives for a moment.
        ...
        // Both price and countdown are drawn inside ONE marker whose center
        // is the exact Y position of the current live price.
