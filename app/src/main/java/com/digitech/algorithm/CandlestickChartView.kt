...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Crosshair (price + time guide) is visible only while the finger is
    // physically down on the chart.
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                if (activeTool != DrawingTool.CROSSHAIR) {
                    ...
                    // Show BOTH time and price guide lines together as soon
                    // as the finger is pressed on the chart.
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (activeTool != DrawingTool.CROSSHAIR) {
                    ...
                    if (
                        ...
                        // Do not let live tick updates re-center the chart once
                        // the finger has actually started moving. A small tap
                        // remains a tap and keeps normal live-follow behavior.
                        if (manualPanCandidate &&
                            (kotlin.math.abs(dx) > 6f || kotlin.math.abs(dy) > 6f)
                        ) {
                            followLive = false
                            manualPanCandidate = false
                        }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                // Crosshair is transient: it disappears as soon as the finger
                // is lifted. Both horizontal (price) and vertical (time) lines
                // therefore always have the same lifetime.
