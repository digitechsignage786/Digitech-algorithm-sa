...
class CandlestickChartView(context: Context) : View(context) {
    ...
    fun clearDrawings() {
        ...
        // Keep the current/open candle at the right side of the chart,
        // like a live trading chart. New ticks therefore remain visible
        // without the user having to pan manually.
        if (candles.isNotEmpty()) {
            val last = candles.lastIndex
            val latestRight =
                chartLeft() + last * candleStep() + candleWidth()
            offsetX = chartRight() - latestRight - 8f
        }
