...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private val liveClockPaint = Paint(Paint.ANTI_ALIAS_FLAG)
    ...
    init {
        ...
        liveClockPaint.color = Color.WHITE
        liveClockPaint.textSize = 11f
        liveClockPaint.textAlign = Paint.Align.CENTER
    ...
    private fun drawTimeScale(
        ...
        // Always show the current clock at the live edge when there is room.
        val nowText = sdf.format(java.util.Date(System.currentTimeMillis()))
        canvas.drawText(nowText, right - 8f, height - 14f, textPaint)
    }

    private fun drawLiveClock(canvas: Canvas) {
        if (candles.isEmpty()) return
        ...
        val latestTime = candles[candles.lastIndex].time
        val elapsed = (now - latestTime).coerceIn(0L, interval)
        val fraction = elapsed.toFloat() / interval.toFloat()

        // The live edge is fixed at the right side while the chart timeline
        // advances continuously between candle boundaries. This is only a
        // visual time marker; it never creates synthetic OHLC data.
        val x = chartRight() - 8f

        crosshairPaint.strokeWidth = 1f
        crosshairPaint.pathEffect = DashPathEffect(floatArrayOf(5f, 5f), 0f)
        crosshairPaint.color = Color.rgb(70, 95, 120)
        canvas.drawLine(
            x,
            chartTop(),
            x,
        ...
        val boxWidth = 72f
        val boxHeight = 22f
        val left = (x - boxWidth / 2f).coerceAtLeast(chartLeft())
        val top = chartBottom() + 4f
        val right = (left + boxWidth).coerceAtMost(chartRight())
