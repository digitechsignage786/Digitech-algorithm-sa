...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun visibleRange(): Pair<Float, Float> {
        ...
        // IMPORTANT:
        // The live price itself must be part of the visible price scale.
        // Otherwise a fast tick can be outside the candle high/low range and
        // the price label/countdown would appear at a different/clamped level.
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        // The countdown is intentionally attached to the CURRENT PRICE LEVEL.
        // As the live price moves, this label moves with the price line.
        ...
        val labelWidth = if (interval >= 60L * 60L * 1000L) 112f else 94f
        val labelHeight = 30f
        val labelLeft = chartRight() - labelWidth
        // Keep the label centered exactly on the live-price Y coordinate.
        // The scale above already includes the live price, so this does not
        // need to be clamped to a different price level.
    ...
    private fun drawTimeScale(
        ...
        // The selected-timeframe countdown is rendered at the live price level
        // inside drawCurrentPrice(), so it moves with the current price.
        textPaint.color = Color.rgb(145, 158, 175)
        textPaint.textSize = 11f
        textPaint.textAlign = Paint.Align.CENTER
