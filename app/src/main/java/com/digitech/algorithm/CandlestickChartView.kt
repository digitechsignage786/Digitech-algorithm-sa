...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live chart clock: refreshed independently of candle aggregation so the
    // visible time keeps moving every second, like a live trading chart.
    ...
    private fun drawLiveClock(
        ...
        textPaint.color = Color.rgb(55, 205, 155)
    ...
    fun setCandles(
        ...
        // Keep the newest/forming candle visible at the right side of the
        // chart. This is important when setCandles() is called on every live
        // price tick: the current candle moves in-place instead of disappearing
        // off-screen.
        val latestIndex = candles.lastIndex
        val latestX =
            chartLeft() +
            latestIndex * candleStep() +
            candleWidth()

        val desiredOffset =
            chartRight() - latestX - 12f

        offsetX = desiredOffset
        targetOffsetX = desiredOffset
        clampHorizontalOffset()
        targetOffsetX = offsetX
