...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live chart clock. It advances every second independently of incoming
    // ticks, so the time scale keeps moving like a live trading chart.
    private val liveClockHandler = Handler(Looper.getMainLooper())
    private var liveClockMillis = System.currentTimeMillis()
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            liveClockMillis = System.currentTimeMillis()
            invalidate()
    ...
    private fun drawTimeScale(
        ...
        val widthOfCandle = candleWidth()
        val step = candleStep()
        val every = max(1, (82f / step).toInt())
        ...
        // A continuously moving live time label. This is deliberately tied to
        // the device clock rather than the last tick, so seconds keep running
        // even when the feed is momentarily quiet.
        val liveSdf = java.text.SimpleDateFormat(
            "HH:mm:ss",
            java.util.Locale.US
        )
        val liveText = liveSdf.format(
            java.util.Date(liveClockMillis)
        )

        val liveX = chartRight() - 26f
        val liveY = height - 2f
        ...
        textPaint.color = Color.rgb(215, 225, 238)
        textPaint.textSize = 9f
        canvas.drawText(
            liveText,
            liveX,
        ...
        // Current candle countdown: the remaining minutes/seconds are shown
        // beside the live clock, giving the user a TradingView-style sense of
        // where the forming candle is inside its timeframe.
        val latest = candles.lastOrNull()
        if (latest != null) {
            val candleDuration = detectCandleDurationMillis()
            val elapsed = (liveClockMillis - latest.time).coerceAtLeast(0L)
            val remaining = (candleDuration - (elapsed % candleDuration))
                .coerceIn(0L, candleDuration)
            ...
            val countX = chartRight() - 82f
            val countY = height - 2f

            boxPaint.color = Color.rgb(18, 25, 35)
            canvas.drawRect(
                countX - 27f,
                countY - 15f,
                countX + 27f,
            ...
            textPaint.color = Color.rgb(30, 200, 140)
            textPaint.textSize = 9f
            canvas.drawText(
                countdown,
                countX,
    ...
    private fun detectCandleDurationMillis(): Long {
        if (candles.size >= 2) {
            val delta = candles[candles.lastIndex].time -
                    candles[candles.lastIndex - 1].time
            if (delta in 30_000L..24L * 60L * 60L * 1000L) {
                return delta
            }
        }

        // Safe fallback for the app's default 5m chart.
        return 5L * 60L * 1000L
    }
