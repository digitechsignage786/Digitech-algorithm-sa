...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun anchorLineToScreen(line: LineData): LineData =
        LineData(
            candlePosToScreenX(line.x1), priceToScreenY(line.y1),
            candlePosToScreenX(line.x2), priceToScreenY(line.y2)
        )

    private fun anchorRectToScreen(rect: RectData): RectData =
        RectData(
            candlePosToScreenX(rect.left), priceToScreenY(rect.top),
            candlePosToScreenX(rect.right), priceToScreenY(rect.bottom)
        )

    private fun anchorMeasureToScreen(m: MeasureData): MeasureData =
        MeasureData(
            candlePosToScreenX(m.x1), priceToScreenY(m.y1),
            candlePosToScreenX(m.x2), priceToScreenY(m.y2)
        )
    ...
    private fun drawStoredDrawings(
        ...
        for (stroke in brushStrokes) {
            for (i in 1 until stroke.size) {
                ...
                val b = stroke[i]
                canvas.drawLine(
                    candlePosToScreenX(a.first), priceToScreenY(a.second),
                    candlePosToScreenX(b.first), priceToScreenY(b.second),
                    drawingPaint
                )
            }
        }
        ...
        for (point in textMarkers) {
            canvas.drawText("TEXT", candlePosToScreenX(point.first), priceToScreenY(point.second), labelPaint)
        }
    }
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (activeTool != DrawingTool.CROSSHAIR &&
                    ...
                    when (activeTool) {
                        DrawingTool.TREND_LINE -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            trendLines.add(LineData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.RAY -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            rayLines.add(LineData(a.first, a.second, b.first, b.second))
                        }
                        ...
                        DrawingTool.VERTICAL_LINE ->
                            verticalLines.add(screenToCandlePos(endX))

                        DrawingTool.RECTANGLE -> {
                            val a = screenPointToAnchor(min(startX, endX), min(startY, endY))
                            val b = screenPointToAnchor(max(startX, endX), max(startY, endY))
                            rectangles.add(RectData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.CIRCLE -> {
                            val a = screenPointToAnchor(min(startX, endX), min(startY, endY))
                            val b = screenPointToAnchor(max(startX, endX), max(startY, endY))
                            circles.add(RectData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.ARROW -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            arrows.add(LineData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.PARALLEL_CHANNEL -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            parallelChannels.add(LineData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.FIBONACCI -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            fibonacciLines.add(LineData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.MEASURE -> {
                            val a = screenPointToAnchor(startX, startY)
                            val b = screenPointToAnchor(endX, endY)
                            measurements.add(MeasureData(a.first, a.second, b.first, b.second))
                        }

                        DrawingTool.BRUSH -> {
                            if (activeBrush.size > 1) {
                                brushStrokes.add(activeBrush.map { screenPointToAnchor(it.first, it.second) })
                            }
                            activeBrush.clear()
                        }

                        DrawingTool.TEXT -> {
                            textMarkers.add(screenPointToAnchor(endX, endY))
                        }
    ...
    private fun convertLegacyDrawingsToAnchors() {
        if (!legacyDrawingsNeedConversion || candles.isEmpty()) return

        fun line(l: LineData): LineData {
            val a = screenPointToAnchor(l.x1, l.y1)
            val b = screenPointToAnchor(l.x2, l.y2)
            return LineData(a.first, a.second, b.first, b.second)
        }
        fun rect(r: RectData): RectData {
            val a = screenPointToAnchor(r.left, r.top)
            val b = screenPointToAnchor(r.right, r.bottom)
            return RectData(a.first, a.second, b.first, b.second)
        }
        fun measure(m: MeasureData): MeasureData {
            val a = screenPointToAnchor(m.x1, m.y1)
            val b = screenPointToAnchor(m.x2, m.y2)
            return MeasureData(a.first, a.second, b.first, b.second)
        }

        for (i in trendLines.indices) trendLines[i] = line(trendLines[i])
        for (i in rayLines.indices) rayLines[i] = line(rayLines[i])
        for (i in horizontalLines.indices) horizontalLines[i] = screenToPrice(horizontalLines[i])
        for (i in verticalLines.indices) verticalLines[i] = screenToCandlePos(verticalLines[i])
        ...
        for (i in brushStrokes.indices) {
            brushStrokes[i] = brushStrokes[i].map { screenPointToAnchor(it.first, it.second) }
        }
        for (i in textMarkers.indices) textMarkers[i] = screenPointToAnchor(textMarkers[i].first, textMarkers[i].second)

        legacyDrawingsNeedConversion = false
        saveDrawings()
    }
    ...
    private fun loadDrawings() {
        ...
        try {
            ...
            legacyDrawingsNeedConversion = version < 2
            val oldW = root.optDouble("width", 0.0).toFloat()
            val oldH = root.optDouble("height", 0.0).toFloat()
            if (oldW <= 0f || oldH <= 0f) return
            val sx = if (version >= 2) 1f else if (width > 0) width / oldW else 1f
            val sy = if (version >= 2) 1f else if (height > 0) height / oldH else 1f
            ...
            if (version >= 2) legacyDrawingsNeedConversion = false
        } catch (_: Exception) { }
    }
