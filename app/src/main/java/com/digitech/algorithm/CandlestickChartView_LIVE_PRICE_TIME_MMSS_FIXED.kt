...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Real-time candle countdown shown beside the live price. This is only
    // a clock/countdown; it never creates synthetic price data.
    private var livePriceForLabel = Float.NaN
    private var liveRemainingMillis = 0L
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        val price = if (livePriceForLabel.isFinite()) {
            livePriceForLabel
        } else {
            candle.close
        }
        ...
        // Price + live candle countdown. The countdown is attached to the
        // current price marker, so it visibly runs MM:SS while the candle
        // is forming, just like a professional live chart.
        val countdownText = formatRemaining(liveRemainingMillis)
        ...
        labelPaintLocal.color = Color.rgb(190, 205, 220)
    ...
    fun setLivePriceCountdown(price: Float, remainingMillis: Long) {
        livePriceForLabel = price
        liveRemainingMillis = remainingMillis.coerceAtLeast(0L)
        invalidate()
    }

    fun setLiveTime(nowMillis: Long, intervalMillis: Long) {
