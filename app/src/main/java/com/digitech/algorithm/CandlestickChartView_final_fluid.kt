...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private val scaleDetector = ScaleGestureDetector(
        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScale(
                ...
                if (!rawFactor.isFinite() || rawFactor <= 0f) {
                    return true
                }

                val oldZoom = zoom
                val oldStep = candleStepForZoom(oldZoom)
                ...
                val focusX =
                    detector.focusX.coerceIn(left, right)

                // Convert the pinch center into candle-space before changing
                // the scale. This keeps the exact point under the fingers fixed.
                val logicalPosition =
                    (focusX - left - offsetX) / oldStep

                // Apply the detector's continuous scale directly. A very small
                // damping factor removes touch-sampling noise without creating
                // the lag caused by a second animation loop.
                val dampedFactor =
                    rawFactor.coerceIn(0.92f, 1.08f)

                val newZoom =
                    (oldZoom * dampedFactor)
                        .coerceIn(0.55f, 5f)

                val newStep =
                    candleStepForZoom(newZoom)
                ...
                offsetX =
                    focusX -
                    left -
                    logicalPosition * newStep

                clampHorizontalOffset()

                // Keep targets synchronized so there is no delayed catch-up.
                targetZoom = zoom
    ...
    private fun startSmoothZoom() {
        ...
        // Pinch zoom is now applied directly in onScale() so there is no
        // delayed animation competing with the user's fingers.
        targetZoom = zoom
        targetOffsetX = offsetX
        smoothZoomRunning = false
    }
