    ...
    private val scaleDetector = ScaleGestureDetector(
        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScale(
                ...
                // Keep the chart under the fingers while zooming, then apply
                // the same safety bounds used by one-finger pan. This prevents
                // pinch zoom from pushing the candle field outside the chart.
                if (newZoom != oldZoom) {
                    val zoomRatio = newZoom / oldZoom
                    offsetX = focusX - (focusX - offsetX) * zoomRatio
                    scaleDrawings(zoomRatio, 1f, focusX, focusY)
                    zoom = newZoom
                }
                ...
                // Save the intended post-zoom position before bounds clamp.
                val intendedOffsetX = offsetX
                val intendedPricePan = pricePan

                // Never let pinch zoom create an uncontrolled outward drift or
                // a completely empty chart. If the bounds correct the chart
                // position, move drawings by exactly the same correction so
                // they stay locked to the chart.
                clampFreePan()

                val correctionDx = offsetX - intendedOffsetX
                val correctionDy = pricePanToScreenDy(pricePan - intendedPricePan)
                if (kotlin.math.abs(correctionDx) > 0.001f) {
                    // correctionDx is the difference between the bounded and
                    // intended horizontal position.
                    translateDrawings(correctionDx, 0f)
                }
                if (kotlin.math.abs(correctionDy) > 0.001f) {
                    // Vertical correction is likewise applied to drawings.
                    translateDrawings(0f, correctionDy)
                }
