        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (activeTool == DrawingTool.CROSSHAIR && fingerDown && dragging) {
                    ...
                    if (kotlin.math.hypot(totalDx.toDouble(), totalDy.toDouble()) > 1.0) {
                        ...
                        // Do NOT move stored drawings in screen-space here.
                        // syncDrawingsToChartTransform() re-projects them from
                        // their candle/time + price coordinates on the next
                        // frame. Applying both transforms caused drawings to
                        // drift/jump when the chart was panned.
                        lastX = event.x
                        lastY = event.y
                    }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (activeTool != DrawingTool.CROSSHAIR &&
                    ...
                    val endX = event.x
                    val endY = event.y
                    val drawingMoved = kotlin.math.hypot(
                        (endX - startX).toDouble(),
                        (endY - startY).toDouble()
                    ) > 3.0

                    // Point-based tools can be placed with a tap. Geometric
                    // tools require a real drag so accidental taps do not
                    // create zero-size drawings.
                    if (!drawingMoved && activeTool !in setOf(
                            DrawingTool.HORIZONTAL_LINE,
                            DrawingTool.VERTICAL_LINE,
                            DrawingTool.TEXT
                        )) {
