...
class MainActivity : AppCompatActivity() {
    ...
    // Advances the visible chart time once per second. Real websocket ticks
    // still control OHLC; this only makes the minute/second time visibly move.
    private val liveClockHandler = Handler(Looper.getMainLooper())
    private val liveClockRunnable = object : Runnable {
        override fun run() {
            if (::chart.isInitialized && !isFinishing && !isDestroyed) {
                chart.setLiveTime(System.currentTimeMillis())
...
super.onCreate(savedInstanceState)
        ...
        // Start the visible minute/second clock immediately.
