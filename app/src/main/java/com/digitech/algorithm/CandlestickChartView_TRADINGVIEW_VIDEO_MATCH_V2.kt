...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawVolume(canvas: Canvas) {
        ...
        var maxVolume = 0f
        for (i in from..to) {
            maxVolume = maxOf(maxVolume, candles[i].volume)
        }
        if (maxVolume <= 0f) return

        val base = chartBottom()
        val top = volumeTop()
        val barHeight = (base - top).coerceAtLeast(4f)
        val width = candleWidth().coerceAtLeast(2f)

        for (i in from..to) {
            val c = candles[i]
            if (c.volume <= 0f) continue

            val x = chartLeft() + i * candleStep() + offsetX
            if (x + width < chartLeft() || x > chartRight()) continue

            val h = (c.volume / maxVolume * barHeight).coerceIn(1f, barHeight)
            val paint = if (c.close >= c.open) upPaint else downPaint
            paint.alpha = 120
            canvas.drawRect(x, base - h, x + width, base, paint)
            paint.alpha = 255
        }
    }
