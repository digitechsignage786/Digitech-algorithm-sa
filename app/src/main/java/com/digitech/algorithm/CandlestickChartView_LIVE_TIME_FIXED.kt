...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawTimeScale(
        ...
        // Historical candle times remain attached to their candles.
        ...
        // LIVE CURRENT TIME:
        // Keep the open/current candle at the right side and continuously
        // advance the time label once per second, without creating fake OHLC.
        val last = candles.last()
        val candleInterval = if (candles.size >= 2) {
            (candles[candles.lastIndex].time -
        ...
        val currentCandleStart =
            (liveNowMillis / candleInterval) * candleInterval

        val currentTimeX =
            (chartRight() - widthOfCandle / 2f)
                .coerceIn(chartLeft(), chartRight())

        // Small live time marker directly below the current/open candle.
        val liveLabel = sdf.format(
            java.util.Date(
                max(currentCandleStart, last.time)
            )
        )
        ...
        // Restore normal label paint state.
    ...
    fun setCandles(
        ...
        if (candles.isNotEmpty()) {
            ...
            // Keep the open/latest candle at the right edge so new ticks
            // update in place instead of making the chart jump.
