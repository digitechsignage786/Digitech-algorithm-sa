...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private var liveIntervalMs = 5L * 60L * 1000L
    ...
    // Fallback countdown derived from the local clock. The activity can
    // override it with server-time countdowns, but this keeps the badge
    // visibly moving even between quiet WebSocket ticks.
    private fun effectiveCountdownMs(): Long {
        if (liveCountdownMs >= 0L) return liveCountdownMs.coerceAtLeast(0L)
        val interval = liveIntervalMs.coerceAtLeast(1L)
        val now = liveNowMillis
        val elapsed = ((now % interval) + interval) % interval
        return (interval - elapsed).coerceIn(0L, interval)
    }
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        val labelWidth = 58f
        val showLiveTime = livePrice.isFinite()
        val labelHeight = if (showLiveTime) 42f else 18f
        ...
        if (showLiveTime) {
            val countdownMs = effectiveCountdownMs()
            val totalSeconds = (countdownMs / 1000L).coerceAtLeast(0L)
            ...
            val clockFormat = java.text.SimpleDateFormat(
                "HH:mm:ss",
                java.util.Locale.US
            )
            labelPaintLocal.textSize = 8f
            labelPaintLocal.color = Color.rgb(150, 165, 180)
            canvas.drawText(
                clockFormat.format(java.util.Date(liveNowMillis)),
                labelLeft + labelWidth / 2f,
                labelTop + 37f,
                labelPaintLocal
            )
     ...
     */
    /** Set the selected chart timeframe interval used by the live countdown. */
    fun setLiveTimeframe(intervalMs: Long) {
        liveIntervalMs = intervalMs.coerceAtLeast(1L)
        lastCandleIntervalMillis = liveIntervalMs
        postInvalidateOnAnimation()
    }

    /** Set the symbol shown next to the live price when needed. */
    fun setLiveSymbol(symbol: String) {
        liveSymbol = symbol.replace("/", "")
        postInvalidateOnAnimation()
    }

    /** Keep the chart clock aligned with the latest market/server tick time. */
    fun setLiveTickTime(timeMs: Long) {
        if (timeMs > 0L) {
            liveNowMillis = timeMs
            postInvalidateOnAnimation()
        }
    }
