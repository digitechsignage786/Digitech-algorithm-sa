...
class CandlestickChartView(context: Context) : View(context) {
    ...
    fun setCandles(
        ...
        // Always open a newly loaded series at the newest candle.
        // The API data is chronological, so the latest candle is the
        // last item in the list. This prevents the chart from opening
        // on the oldest candles.
        if (candles.isNotEmpty()) {
            ...
            // Keep the latest candle visible with a small right margin.
            offsetX += 8f
        } else {
            offsetX = 0f
        }
