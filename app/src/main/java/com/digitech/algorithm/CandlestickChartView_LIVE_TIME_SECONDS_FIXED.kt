...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun liveTimelineShift(): Float {
        ...
        // The chart uses the actual candle open timestamp. Once the current
        // candle is forming, let the chart advance continuously from 0..1 of
        // that timeframe. No synthetic candle/price is created here.
    ...
    private fun drawTimeScale(
        ...
        // Live clock: show minutes AND seconds at the live edge. This is
        // intentionally independent of candle creation, so the clock keeps
        // moving every second even while the same 5m/15m candle is forming.
        val now = System.currentTimeMillis()
        val minuteText = sdf.format(java.util.Date(now))
        val secondSdf = java.text.SimpleDateFormat("HH:mm:ss", java.util.Locale.US)

        textPaint.textAlign = Paint.Align.RIGHT
        textPaint.textSize = 11f
        canvas.drawText(minuteText, right - 8f, height - 14f, textPaint)

        textPaint.textSize = 9f
        textPaint.color = Color.rgb(95, 170, 220)
        canvas.drawText(
            secondSdf.format(java.util.Date(now)),
            right - 8f,
            height - 28f,
            textPaint
        )
    ...
    fun setCandles(
        ...
        // Re-anchor only when the first dataset arrives or a NEW candle starts.
        // Do not re-anchor on every tick: doing that cancels the smooth
        // second-by-second movement of the current forming candle.
        if (wasEmpty || newLastTime != oldLastTime) {
            keepLatestAtRight()
        }
