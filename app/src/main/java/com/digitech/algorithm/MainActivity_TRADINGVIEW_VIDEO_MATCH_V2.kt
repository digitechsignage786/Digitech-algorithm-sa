...
super.onCreate(savedInstanceState)
    ...
    private fun buildUI() {
        ...
        // MAIN AREA: the supplied TradingView videos keep the chart
        // full-width. Drawing/tools are exposed from the compact row below,
        // so no permanent left/right rail steals chart space.
        ...
        chartTool(selectedSymbol.replace("/", ""), 82) { showSymbolDialog() }
        chartTool(selectedTimeframe, 50)
        chartTool("✎", 44) { chart.setDrawingTool(DrawingTool.TREND_LINE) }
        chartTool("▥", 44) { showTools() }
        chartTool("▦", 44) { showIndicators() }
        chartTool("⌁", 44) { showTools() }
        chartTool("＋", 44) { chart.setDrawingTool(DrawingTool.CROSSHAIR) }
        chartTool("◷", 44) { showReplay() }
        chartTool("▥", 44) { showBrokers() }
        chartTool("◀◀", 48) { stepReplay() }
        chartTool("▱", 44) { showTools() }
