...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawLivePriceTimeBadge(canvas: Canvas) {
        if (!livePrice.isFinite() || liveCountdownMs < 0L || candles.isEmpty()) {
            return
        }
        ...
        // Price line follows the live tick exactly.
        ...
        // Compact black badge at the right price axis, matching the
        // reference video: SYMBOL / PRICE on top and MM:SS underneath.
        val badgeWidth = 106f
        val badgeHeight = 48f
        val left = (width - badgeWidth - 2f).coerceAtLeast(chartLeft())
        val right = width - 2f
        val top = (priceY - badgeHeight / 2f)
            .coerceIn(4f, height - badgeHeight - 4f)
        ...
        canvas.drawRoundRect(
            left, top, right, bottom, 3f, 3f, badgePaint
        )
    ...
    fun setLiveTickTime(timeMs: Long) {
        // The MainActivity owns the server-time clock. Keeping this method
        // makes the chart API compatible with the live stream without
        // inventing any market time.
        invalidate()
    }
