...
super.onCreate(savedInstanceState)
                ...
                override fun onOpen(
                    ...
                    runOnUiThread {
                        if (serial == requestSerial) {
                            ...
                            streamStatusText.text = "WEBSOCKET: CONNECTED — waiting for live price tick"
                ...
                when {
                    status.equals("ok", ignoreCase = true) ||
                        ...
                        streamStatusText.text =
                            "WEBSOCKET: SUBSCRIBED — " + symbol + " | waiting for live price tick"
                        streamStatusText.setTextColor(Color.rgb(30, 200, 140))
                    }
    ...
    private fun handleLivePrice(
        ...
        try {
            ...
            // Twelve Data sends the actual market tick timestamp in seconds.
            // Keep the tick timestamp intact for live price updates, but use a
            // timeframe-aligned candle-open time when aggregating OHLC.
    ...
    private fun updateCurrentCandle(
        ...
        synchronized(liveLock) {
            ...
            // A candle is anchored to the timeframe boundary. The incoming
            // tick itself remains the real market price; only the candle's
            // OPEN TIME is normalized to the correct minute boundary.
            val candleOpenTime = (tickTime / interval) * interval

            // Every real tick updates the currently forming candle. When the
            // boundary changes, start a new candle from that real tick.
            val sameCandle = current.time == candleOpenTime
            ...
            if (sameCandle) {
                ...
                if (candleOpenTime > current.time) {
