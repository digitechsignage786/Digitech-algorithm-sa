...
class CandlestickChartView(context: Context) : View(context) {
    ...
    private fun drawCurrentPrice(canvas: Canvas) {
        ...
        // Live price box: price + continuously changing candle countdown.
        // This mirrors the behaviour shown in the reference video: the
        // remaining time changes every second even when no new price tick
        // arrives, while the price itself comes from the real latest tick.
        ...
        labelPaintLocal.textSize = 9f
        labelPaintLocal.color = Color.rgb(185, 205, 225)
        canvas.drawText(
            liveCountdownText(),
            labelLeft + labelWidth / 2f,
            labelTop + 24f,
    ...
    private fun liveCountdownText(): String {
        val candle = candles.lastOrNull() ?: return "--:--"

        val interval = lastCandleIntervalMillis.coerceAtLeast(60_000L)
        val endTime = candle.time + interval
        val remaining = (endTime - liveNowMillis).coerceAtLeast(0L)
        val totalSeconds = ((remaining + 999L) / 1000L).toLong()
    ...
    private fun drawTimeScale(
        ...
        // The live edge includes seconds so the timeline visibly advances
        // minute-by-minute and second-by-second.
        val liveClock = java.text.SimpleDateFormat(
            "HH:mm:ss",
            java.util.Locale.US
        )
        val liveText = liveClock.format(java.util.Date(liveNowMillis))
