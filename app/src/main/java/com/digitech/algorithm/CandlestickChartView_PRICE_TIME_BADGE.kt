...
class CandlestickChartView(context: Context) : View(context) {
    ...
    // Live TradingView-style price/time badge. The price comes only from
    // the real market tick; the MM:SS countdown is driven by MainActivity
    // from the selected candle interval and server-aligned time.
    private var livePrice = Float.NaN
    private var liveCountdownMs = -1L
    private var liveSymbol = "EURUSD"
    private var liveIntervalMs = 5 * 60_000L

    // Continuous live clock. It keeps HH:mm:ss moving even between ticks.
    private var liveNowMillis = System.currentTimeMillis()
    private val liveClockHandler = android.os.Handler(android.os.Looper.getMainLooper())
    ...
    private fun drawLivePriceTimeBadge(canvas: Canvas) {
        if (!livePrice.isFinite() || liveCountdownMs < 0L || candles.isEmpty()) return

        val range = visibleRange()
        val priceY = priceToY(livePrice, range.first, range.second)
            .coerceIn(24f, height - 74f)

        val totalSeconds = max(0L, liveCountdownMs) / 1000L
        val minutes = totalSeconds / 60L
        val seconds = totalSeconds % 60L
        val timerText = String.format(java.util.Locale.US, "%02d:%02d", minutes, seconds)
        val priceText = String.format(java.util.Locale.US, "%.5f", livePrice)
        val clockText = java.text.SimpleDateFormat("HH:mm:ss", java.util.Locale.US)
            .format(java.util.Date(liveNowMillis))

        val guidePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(95, 108, 125)
            strokeWidth = 1f
            pathEffect = DashPathEffect(floatArrayOf(5f, 5f), 0f)
        }
        canvas.drawLine(chartLeft(), priceY, chartRight(), priceY, guidePaint)

        // The badge is attached to the live price so both PRICE and time
        // visibly travel with the current market price.
        val badgeWidth = 106f
        val badgeHeight = 66f
        val left = (width - badgeWidth - 2f).coerceAtLeast(chartLeft())
        val right = width - 2f
        val top = (priceY - badgeHeight / 2f).coerceIn(4f, height - badgeHeight - 4f)

        val bg = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.rgb(8, 8, 10) }
        canvas.drawRoundRect(left, top, right, top + badgeHeight, 3f, 3f, bg)

        val symbolPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.WHITE
            textSize = 11f
        }
        canvas.drawText(liveSymbol, left + 7f, top + 15f, symbolPaint)
        ...
        canvas.drawText(priceText, right - 7f, top + 15f, pricePaint)

        val clockPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(120, 210, 170)
            textSize = 13f
            textAlign = Paint.Align.RIGHT
        }
        canvas.drawText(clockText, right - 7f, top + 34f, clockPaint)
        ...
        canvas.drawText("BAR $timerText", right - 7f, top + 53f, timerPaint)
    }

    /** Set the selected timeframe duration used by the live time badge. */
    fun setLiveTimeframe(intervalMs: Long) {
        liveIntervalMs = intervalMs.coerceAtLeast(1L)
        invalidate()
    }

    /** Set the symbol displayed in the live price/time badge. */
    fun setLiveSymbol(symbol: String) {
        liveSymbol = symbol.replace("/", "")
        invalidate()
    }

    /** Keep the latest exchange/server tick timestamp available to the chart. */
    fun setLiveTickTime(timeMs: Long) {
        if (timeMs > 0L) liveNowMillis = timeMs
        postInvalidateOnAnimation()
    }

    /** Update the real tick price and the remaining MM:SS time. */
    fun setLivePriceCountdown(price: Float, remainingMs: Long) {
        livePrice = price
        liveCountdownMs = remainingMs.coerceIn(0L, liveIntervalMs)
        postInvalidateOnAnimation()
    }
