    ...
    private val scaleDetector = ScaleGestureDetector(
        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScale(
                ...
                // Use a small, stable response to the pinch. The important
                // part is that the point between the fingers remains fixed;
                // the chart must not fly toward an edge while zooming.
                val factor = (1f + (rawFactor - 1f) * 0.55f)
                    .coerceIn(0.94f, 1.06f)

                if (kotlin.math.abs(factor - 1f) < 0.0005f) return true
                ...
                // Capture the price mapping BEFORE changing either zoom.
                // visibleRange() depends on zoom, so calculating the anchor
                // after changing zoom causes the chart to jump.
                val before = visibleRange()
                val oldHigh = before.first
                val oldLow = before.second
                val oldRange = (oldHigh - oldLow).coerceAtLeast(0.0000005f)
                val oldCenter = (oldHigh + oldLow) * 0.5f
                val oldHalf = oldRange * 0.5f
                val focusFraction =
                    ((focusY - chartTop()) /
                        (chartBottom() - chartTop()).coerceAtLeast(1f))
                        .coerceIn(0f, 1f)
                val focusPrice = oldHigh - focusFraction * oldRange

                val newZoom = (oldZoom * factor).coerceIn(0.45f, 5f)
                val newPriceZoom = (oldPriceZoom * factor).coerceIn(0.55f, 6f)

                // Horizontal pinch: keep the exact finger midpoint anchored.
                if (newZoom != oldZoom) {
                    val ratio = newZoom / oldZoom
                    offsetX = focusX - (focusX - offsetX) * ratio
                    scaleDrawings(ratio, 1f, focusX, focusY)
                    zoom = newZoom
                }

                // Vertical pinch: keep the price underneath the finger midpoint
                // at the same screen position. Do NOT recompute this from the
                // newly zoomed visible candle set.
                if (newPriceZoom != oldPriceZoom) {
                    val newHalf = oldHalf * (oldPriceZoom / newPriceZoom)
                    val newCenter =
                        focusPrice - (1f - 2f * focusFraction) * newHalf

                    // visibleRange() uses center = baseCenter + pricePan.
                    // Recover the old base center from the old mapping.
                    val oldBaseCenter = oldCenter - pricePan
                    pricePan = newCenter - oldBaseCenter
                    priceZoom = newPriceZoom

                    scaleDrawings(1f, newPriceZoom / oldPriceZoom, focusX, focusY)
                }

                // Apply only a small safety correction after the zoom. The
                // correction is based on the final zoomed geometry, so it cannot
                // create the large outward jump caused by the previous patch.
                val beforeClampX = offsetX
                val beforeClampPan = pricePan
                clampFreePan()

                val correctionDx = offsetX - beforeClampX
                val correctionDy = pricePanToScreenDy(pricePan - beforeClampPan)
                if (kotlin.math.abs(correctionDx) > 0.001f ||
                    kotlin.math.abs(correctionDy) > 0.001f
                ) {
                    translateDrawings(correctionDx, correctionDy)
                }
